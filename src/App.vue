    ...
    <header class="sticky top-0 z-30 backdrop-blur-md bg-pit-bg/70 border-b border-pit-border">
      <div class="max-w-5xl mx-auto px-4 md:px-6 py-3 flex items-center gap-3">
        ...
        <div class="ml-auto flex items-center gap-2">
          ...
          <button class="pit-btn-ghost !px-3 !py-2 text-sm" @click="onNewQuote" title="Nueva cotización">
            <svg viewBox="0 0 24 24" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <!-- Step indicator -->
      <div class="max-w-5xl mx-auto px-4 md:px-6 pb-4">
    ...
    <main class="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-10 pb-32">
      <div class="stepper-stage">
        <Transition :name="direction === 'forward' ? 'slide' : 'slide'" mode="out-in">
          ...
          <FinalStep v-else key="step-3" @back="goToStep(2)" @reset="onNewQuote" @toast="(msg, type) => showToast(msg, type)" />
        </Transition>
      </div>
    </main>
